...
typedef struct _Nod_ { //Estrutura que define cada nó da lista
	struct _Nod_* next; //Referência para o próximo nó da lista
	struct _Nod_* prev; //Referência para o nó anterior da lista
	int dado; //Dado a ser guardado pelo nó
}Nod; //Nó

typedef struct _Lista_Dupla_ { //Estrutura que define a lista
	Nod* head; //Cabeça da lista (primeiro elemento da lista)
	Nod* tail; //Rabo da lista (último elemento da lista)
}ListaDupla; //Lista
