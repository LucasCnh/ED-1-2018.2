...
Fila_controle* criaFilaControle (int tamanhoN, int tamanhoP, int peso){ //Função que aloca estrutura do tipo Fila_controle assim como seus membros
	...
	if ((filas -> normal == NULL) || (filas -> normal -> valores == NULL)){ //Se a fila normal não pôde ser alocada
	...
	if ((filas -> prioritaria == NULL) || (filas -> prioritaria -> valores == NULL)){ //Se a fila prioritária não pôde ser alocada
		...
		free(filas -> normal -> valores); //Desaloca o vetor da fila normal
		filas -> normal -> valores; //Aponta o ponteiro da mesma á NULL
...
int enqueueControle (Fila_controle* filas, int valor, int prioridade){ //Função que inserirá uma nova pessoa na fila normal ou prioritária de acordo com o valor de prioridade
	...
	if ((filas == NULL) || (filas -> normal == NULL) || (filas -> normal -> valores == NULL)){ //Checando se a fila normal está alocada devidamente assim como sua estrutura e a estrutura filas
		return 0; //Caso não esteja, a função retorna 0, simbolizando que não se pôde inserir a nova pessoa  
	}	

	if ((filas -> prioritaria == NULL) || (filas -> prioritaria -> valores == NULL)){ //Checando se a fila prioritária está alocada devidamente assim como sua estrutura
		return 0; //Caso não esteja, a função retorna 0, simbolizando que não se pôde inserir a nova pessoa  
	}
...
int destroiFilaControle (Fila_controle** filas){
	...
	if ((aux == NULL) || (aux -> normal == NULL) || (aux -> normal -> valores == NULL)){ //Checando se algum dos membros já está desalocado
		return 0;
	}

	if ((aux -> prioritaria == NULL) || (aux -> prioritaria -> valores == NULL)){
