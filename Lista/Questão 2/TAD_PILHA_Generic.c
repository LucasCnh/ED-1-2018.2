...
PilhaG* criaPilhaG (int tamanho, Printar p, Desalocar d){ //Função responsável por criar (alocar) uma nova pilha

	PilhaG* pilha = (PilhaG*) malloc(sizeof(PilhaG));
...
int pushG(PilhaG* pilha, void* valor){ //Função responsável por adicionar um novo valor ao topo da pilha
...
void* popG(PilhaG* pilha){ //Função responsável por remover um valor do topo da pilha
...
void printaPilhaG (PilhaG* pilha){ //Função que printa a pilha
...
int destroiPilhaG (PilhaG** pilha){ //Função responsável por destruir a pilha (desalocar)

	PilhaG* aux = *pilha; //Criando um ponteiro aux do tipo Pilha que apontará para *pilha
