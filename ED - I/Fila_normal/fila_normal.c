...
Fila* criaFila (int tamanho){ //Cria uma nova fila alocando uma estrutura e um vetor que a representará
	...
	fila -> matriculas = (int*) malloc(tamanho*sizeof(int)); //Alocando o vetor que representará a fila com o tamanho dado pelo usuário do TAD

	if (fila -> matriculas == NULL){ //Checando se o vetor fila -> matriculas pôde ser alocado devidamente
...
int enqueue (Fila* fila, int valor){ //Função que insere nova pessoa na fila caso a mesma exista ou não esteja cheia
	...
	if ((fila -> ult) != (fila -> prim)){ //Checando se a fila não está cheia usando o conceito de vetor circular
		fila -> matriculas[fila -> ult] = valor; //Inserindo o valor na ultima posição da fila
...
void printaFila (Fila* fila){ //função que imprime na tela o valor de cada pessoa presente na fila
	...
	if ((fila == NULL) || (fila -> matriculas == NULL)){ //Caso a fila não exista nada é printado
		return;
	}
	...
	do{ //O do - while foi usado pois caso a fila já estivesse cheia o valor de i já seria igual ao valor de fila -> ult
		...
		printf("%i ", fila -> matriculas[i]); //Printa o valor da posição atual de i na fila
...
int destroiFila (Fila** fila){ //Função que destruirá a fila
	...
	if ((aux == NULL) || (aux -> matriculas == NULL)){ //Caso a fila já esteja desalocada a função retorna 0
		return 0; //Simbolizando que não foi possível destruir uma fila inexistente
	}

	free(aux -> matriculas); //Desaloca o vetor aux -> matriculas
	aux -> matriculas = NULL; //Atribui ao ponteiro o valor de NULL
